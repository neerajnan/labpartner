...
async def search_pubmed(term: str, max_results: int = 3) -> list[str]:
    ...
    with timed_step("pubmed.search", term_chars=len(term), max_results=max_results):
...
async def fetch_abstracts(pmids: list[str]) -> str:
    ...
    with timed_step("pubmed.fetch", pmid_count=len(pmids)):
...
async def get_context_for_findings(search_terms: list[str]) -> dict[str, dict[str, Any]]:
    ...
    results = {}
    with timed_step("pubmed.context", term_count=len(search_terms)):
        for term in search_terms:
            ...
            print(
                "timing step=pubmed.term "
                f"status=ok pmid_count={len(pmids)} abstract_chars={len(abstracts)}",
                flush=True,
            )
            await asyncio.sleep(REQUEST_DELAY_SECONDS)
    return results
